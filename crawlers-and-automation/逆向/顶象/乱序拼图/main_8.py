...
from puzzle_test import get_correct_arr
import execjs
...
def run(type):
    ...
    image_tp = "https://static.dingxiang-inc.com/picture" + tp
    orders, move_pos = get_correct_arr(s.get(image_tp).content)
    print(orders)
    print(move_pos)
    start = pos_map[move_pos[0]]
    end = pos_map[move_pos[1]]
    ac = execjs.compile(open('get_ac.js', 'r', encoding='utf-8').read()).call('get_ac_puzzle_move', sid, start, end, orders)
    print(ac)
